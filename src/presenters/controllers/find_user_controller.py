...
class FindUserController:
    ...
    def route(self, http_request: Type[HttpRequest]) -> HttpResponse:
        """Mood to call use case."""
        response = None

        if http_request.query:
            query_string_params = http_request.query.keys()

            if ("user_id" in query_string_params) and (
            ...
            elif ("user_id" in query_string_params) and (
                "user_name" not in query_string_params
            ):
                user_id = http_request["user_id"]
                response = self.find_user_use_case.by_id(user_id=user_id)
            elif ("user_id" not in query_string_params) and (
                "user_name" in query_string_params
            ):
